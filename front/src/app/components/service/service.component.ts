...
export class ServiceComponent implements OnInit {
  ...
  startDate: string = '';
  endDate: string = '';
  selectedTimeFilter: string = 'month'; // Default filter: 'month'
  ...
  onTimeFilterChange(): void {
    // Reset custom dates if a time filter is selected
    this.startDate = '';
    this.endDate = '';
    const serverId = Number(this.route.snapshot.paramMap.get('serverId'));
    this.loadServices(serverId);
  ...
  onDateRangeChange(): void {
    // When startDate and endDate are manually selected
    this.selectedTimeFilter = ''; // Clear time filter
    const serverId = Number(this.route.snapshot.paramMap.get('serverId'));
    this.loadServices(serverId);
  }
